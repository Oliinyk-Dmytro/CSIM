...
to relife-cars [road] ;; turtle procedure
  set speed 0
  set wait-time 0
...
to put-on-start-empty-road [road]  ;; turtle procedure
  move-to road
end

to-report get-start-empty-road  ;; turtle procedure
...
to go
  ...
  [
    ...
    print (word "[DEBUG] go. Is there an empty road: " empty-road)
    ifelse empty-road != nobody
    [
      create-turtles 1 [
        relife-cars empty-road
        set-car-color
        print (word "[DEBUG] go. The turtle is created at X cor = " pxcor "; Y cor = " pycor)
      ]
      set dead-number dead-number - 1
    ]
    [
      print (word "[DEBUG] go. Not enough space. Cars are remaining to be placed: " dead-number)
    ]
  ]
  ...
  ask turtles [
   ...
   print (word "[DEBUG] go. dead number = " dead-number)
...
to set-signal-colors  ;; intersection (patch) procedure
  ifelse power?
  [
    ifelse phase < yellow-signal-ticks
    [
...
to set-car-speed  ;; turtle procedure
  ...
  if patch-ahead 1 != nobody [
    ...
    [
      (ifelse
        pcolor = yellow and (rand <= yellow-drive-chance) [
          ifelse up-car?
          [ set-speed 0 -1 ]
          [ set-speed 1 0 ]
        ]
        [pcolor] of patch-ahead 1 = yellow and (rand > yellow-drive-chance) [
          set speed 0
        ]
        [
          ifelse up-car?
...
to-report is-end-of-world
  (ifelse
    (pxcor = max-pxcor and up-car? = false) [
      print (word "horiz die")
      print (word "x_cor: " pxcor ";y_cor: " pycor)
      print up-car?
      report true
    ]
    (pycor = min-pycor and up-car? = true) [
      print (word "vert die")
      print (word "x_cor: " pxcor ";y_cor: " pycor)
      print up-car?
      report true
